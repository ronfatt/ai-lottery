...
export const PoolView: React.FC = () => {
  const { globalPoolAmountUSDT, poolGrowthTodayUSDT, showToast } = useDemo();

  // Pool values calculated from dynamic live pool in USDT
  const communityPool = globalPoolAmountUSDT * 0.5; // 50% of total pool (5% of GMV)
  const performancePool = globalPoolAmountUSDT * 0.3; // 30% of total pool (3% of GMV)
  const creatorPool = globalPoolAmountUSDT * 0.2; // 20% of total pool (2% of GMV)
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div className="pb-4 border-b border-white/10 space-y-1">
        <div className="flex items-center gap-2 text-xs font-mono text-lime-400 font-bold">
          ...
          <span>GLOBAL REVENUE SHARING // 全网分红池 (USDT)</span>
        </div>
        <h2 className="font-display font-black text-2xl sm:text-4xl text-white">
          ORACLE 全球分红池体系
        </h2>
        <p className="text-xs font-mono text-metal-300">
          基于全网真实付费会员 GMV 自动注入 · 8月周期进行中 · 3 大子分红池透明加权 (USDT)
        </p>
      </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between pb-6 border-b border-white/10 gap-6">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs font-mono text-lime-400">
              ...
              <span className="px-2 py-0.5 rounded text-[9px] bg-lime-400/20 text-lime-400 font-bold">
                今日注资 +{poolGrowthTodayUSDT.toFixed(2)} USDT
              </span>
            </div>

            <div className="font-mono font-black text-4xl sm:text-6xl text-white text-glow-lime tracking-tight">
              {globalPoolAmountUSDT.toLocaleString('en-US', { minimumFractionDigits: 2 })} <span className="text-3xl text-lime-400">USDT</span>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-surface-300/80 border border-lime-400/40 text-right font-mono space-y-1">
            ...
            <span className="font-mono font-black text-3xl text-lime-400">
              {totalPoolEstimated.toFixed(2)} USDT
            </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="p-6 rounded-2xl bg-surface-200/80 border border-lime-400/30 space-y-4">
            ...
            <div className="font-mono font-black text-2xl text-white">
              {communityPool.toLocaleString('en-US', { minimumFractionDigits: 2 })} <span className="text-xs text-metal-400">USDT</span>
            </div>
            ...
            <div className="space-y-1.5 pt-2 border-t border-white/5 font-mono text-xs text-metal-300">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-cyber-blue font-bold">{commShareVal.toFixed(2)} USDT</span>
              </div>
              <div className="flex justify-between">
                <span>您持有份数：</span>
                <span className="text-lime-400 font-bold">8 Shares (大师)</span>
              </div>
              <div className="flex justify-between pt-1 border-t border-white/5 text-lime-400 font-bold">
                <span>预估收益：</span>
                <span>{myCommReward.toFixed(2)} USDT</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-surface-200/80 border border-cyber-blue/30 space-y-4">
            ...
            <div className="font-mono font-black text-2xl text-white">
              {performancePool.toLocaleString('en-US', { minimumFractionDigits: 2 })} <span className="text-xs text-metal-400">USDT</span>
            </div>
            ...
            <div className="space-y-1.5 pt-2 border-t border-white/5 font-mono text-xs text-metal-300">
              ...
              <div className="flex justify-between">
                <span>单份价值：</span>
                <span className="text-cyber-blue font-bold">{perfShareVal.toFixed(2)} USDT</span>
              </div>
              <div className="flex justify-between">
                <span>您持有绩效份数：</span>
              ...
              <div className="flex justify-between pt-1 border-t border-white/5 text-cyber-blue font-bold">
                <span>预估收益：</span>
                <span>{myPerfReward.toFixed(2)} USDT</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-surface-200/80 border border-cyber-violet/30 space-y-4">
            ...
            <div className="font-mono font-black text-2xl text-white">
              {creatorPool.toLocaleString('en-US', { minimumFractionDigits: 2 })} <span className="text-xs text-metal-400">USDT</span>
            </div>
            ...
            <div className="space-y-1.5 pt-2 border-t border-white/5 font-mono text-xs text-metal-300">
              ...
              <div className="flex justify-between pt-1 border-t border-white/5 text-cyber-violet font-bold">
                <span>预估收益：</span>
                <span>{myCreatorReward.toFixed(2)} USDT</span>
              </div>
            </div>
          </div>
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-surface-100/90 border border-white/15 backdrop-blur-xl shadow-glass-card space-y-4 font-mono text-xs">
        <div className="flex items-center justify-between pb-3 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display font-black text-base text-white">
              透明数学分红结算公式 (TRANSPARENT USDT CALCULATION)
            </h3>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-surface-200/80 border border-white/5 space-y-2 text-metal-200">
          ...
          <p className="text-lime-400 text-sm">
            单份收益 (Share Value) = 总社群池金额 (192,140 USDT) ÷ 全网合格加权总份数 (2,436) = 78.88 USDT / 份
          </p>
          <p className="text-white">
            您的预估收益 = 8 Shares × 78.88 USDT = <strong className="text-lime-400">631.04 USDT</strong>
          </p>
        </div>

        <div className="text-[11px] text-metal-400">
          * 提示：当月分红收益将于次月 1 日 00:00 经智能合约自动核验并直接清算入账至您的 USDT 奖金钱包中。
        </div>
      </div>

      {/* Historical Settled Archive Table in USDT */}
          ...
          <table className="w-full text-left font-mono text-xs">
            <thead className="bg-surface-200/60 text-metal-400 text-[10px] uppercase border-b border-white/5">
              <tr>
                ...
                <th className="py-3 px-4">实际清算收益 (USDT)</th>
            ...
            <tbody className="divide-y divide-white/5 text-metal-200">
              {MOCK_POOL_HISTORY.map((hist) => (
                <tr key={hist.month} className="hover:bg-white/5">
                  ...
                  <td className="py-3.5 px-4 font-bold text-white">{hist.poolSizeUSDT.toLocaleString()} USDT</td>
                  <td className="py-3.5 px-4 text-lime-400">{hist.yourShares} 份 (大师)</td>
                  <td className="py-3.5 px-4 text-cyber-blue">{hist.shareValueUSDT.toFixed(2)} USDT</td>
                  <td className="py-3.5 px-4 font-black text-lime-400">{hist.rewardEarnedUSDT.toFixed(2)} USDT</td>
