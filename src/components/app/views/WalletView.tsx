...
export const WalletView: React.FC = () => {
  ...
  const handleWithdraw = () => {
    showToast('演示模式：USDT 提领申请已提交，将于下个结算周期清算至指定地址。', 'info');
  };
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div className="pb-4 border-b border-white/10 space-y-1">
        <div className="flex items-center gap-2 text-xs font-mono text-lime-400 font-bold">
          ...
          <span>USDT REWARD WALLET // USDT 奖金钱包</span>
        </div>
        <h2 className="font-display font-black text-2xl sm:text-4xl text-white">
          USDT 收益与闭环积分钱包
        </h2>
        <p className="text-xs font-mono text-metal-300">
          全网统一采用 USDT 结算 · 纯净无投机代币 · 来源于会员订阅与品牌赞助
        </p>
      </div>

      {/* 2 Big Cards: USDT Cash Balance & Oracle Credits */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-surface-100/90 border-2 border-lime-400/50 backdrop-blur-xl shadow-glow-lime/20 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-lime-400/20 text-lime-400 border border-lime-400/30 font-bold">
              正常可用
            </span>
          </div>
          ...
          <div className="pt-2 flex gap-3 font-mono text-xs">
            <button
              ...
              className="flex-1 py-3.5 rounded-xl bg-lime-400 hover:bg-lime-300 text-black font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all shadow-glow-lime"
            >
              <ArrowUpRight className="w-4 h-4" />
              <span>提领 USDT (WITHDRAW USDT)</span>
            </button>
          </div>
        </div>
      ...
      <div className="p-4 rounded-2xl bg-surface-100 border border-white/10 font-mono text-xs flex items-center gap-3 text-metal-300">
        ...
        <p className="text-[11px] leading-relaxed">
          安全合规声明：本钱包严格遵循合规指引，不涉及任何杠杆合约 (Leverage)、期货对冲 (Futures)、加密货币兑换 (Swap)、质押理财 (Staking) 或博彩下注对赌机制。所有资金流向完全基于真实会员订阅与品牌赞助。
        </p>
      </div>
        ...
        <div className="flex items-center justify-between pb-3 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display font-black text-lg text-white">
              USDT 账目流水明细 (TRANSACTION HISTORY)
            </h3>
          </div>
          ...
          <table className="w-full text-left font-mono text-xs">
            <thead className="bg-surface-200/60 text-metal-400 text-[10px] uppercase border-b border-white/5">
              <tr>
                ...
                <th className="py-3 px-4">金额 (USDT)</th>
